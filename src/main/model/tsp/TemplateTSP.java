...
public abstract class TemplateTSP implements TSP {
	 ...
	 *            : the current node
	 * @param unseenNodes
	 *            : table of nodes we still have to visit.
	 * @param cost
	 *            : cost[i][j] = duration to go from i to j, with 0 <= i <
	 *            numberOfNodes and 0 <= j < numberOfNodes.
	 * @param duration
	 *            : duration[i] = duration to visit i node with 0 <= i <
	 *            numberOfNodes.
	 ...
	 */
	protected abstract Iterator<Integer> iterator(Integer sommetCrt, ArrayList<Integer> unseenNodes, double[][] cost, int[] duration);
	 ...
	 *            table of nodes we still have to visit.
	 * @param seenNodes
	 *            table of nodes we already have visited.
	 * @param seenCost
	 *            the sum of the arcs' cost of the path going through all seen
	 *            nodes, plus the duration of the seen nodes.
	 * @param cost
	 *            : cost[i][j] = duration to go from i to j, with 0 <= i <
	 *            numberOfNodes and 0 <= j < numberOfNodes.
	 * @param duration
	 ...
	 *            numberOfNodes.
	 * @param startingTime
	 *            : moment when resolution started.
	 * @param timelimit
	 *            : timelimit for the resolution.
	 */
	void branchAndBound(int sommetCrt, ArrayList<Integer> unseenNodes, ArrayList<Integer> seenNodes, double seenCost, double[][] cost, int[] duration, long startingTime, int timelimit) {
		...
		if (unseenNodes.size() == 0) {
			...
			while (it.hasNext()) {
				...
				branchAndBound(nextNode, unseenNodes, seenNodes, seenCost + cost[sommetCrt][nextNode] + duration[nextNode], cost, duration, startingTime, timelimit);
