...
public class PlanView extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		this.graphics = g;
		List<Intersection> intersections = plan.graph.values();
	...
	/**
	 * Method called any time we need to draw a plan section.
	 * Draw a line between the two intersections of the section.
	 * @param g the graphics component
	...
	public void printSection(Graphics g, Section section){
		...
		double startLong = section.getStart().getLong();
		double endLat = section.getEnd().getLat();
		double endLong = section.getEnd().getLong();
		/* Normalization */
	...
	/**
	 * Method used to extract max and min of latitudes and longitudes in the current plan.
	 * (Used to normalize the values in order to print the sections).
	 */
	private void findExtremes()	{
		...
		minLat = first.getLat();
		maxLat = first.getLat();
		minLong = first.getLong();
		maxLong = first.getLong();		
		...
		while (it.hasNext()){
			...
			if (current.getLong() < minLong){
				minLong = current.getLong();
			}
			if (current.getLong() > maxLong){
				maxLong = current.getLong();
			}
		}		
	}
}
